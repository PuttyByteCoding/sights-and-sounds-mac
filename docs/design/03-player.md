...
| Paste into a tag field | A paste that looks like title case without its spaces — `BenFolds` off a file name — gets them back, `Ben Folds` (`String.splittingTitleCaseWords`: a word starts at a capital after a lower-case letter, or at the last capital of a run of them before a lower-case letter, so `DMBLive` is `DMB Live`; `Phish`, `ACDC`, `iPhone` and anything with a space are untouched). A change that grows the text by two or more characters is a paste. Applies to the Universal and category fields, the tag sheet's name, the bindings editor's search and the tag pickers. |
