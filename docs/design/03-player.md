...
| Paste, anywhere | ⌘V into any text field in the app: a pasted run that looks like title case without its spaces — `BenFolds` off a file name — gets them back, `Ben Folds` (`String.splittingTitleCaseWords`: a word starts at a capital after a lower-case letter, or at the last capital of a run of them before a lower-case letter, so `DMBLive` is `DMB Live`; `Phish`, `ACDC`, `iPhone` and anything with a space are untouched). One app-wide key monitor (`PasteTitleCaseSplitter`) swaps the pasteboard's string for the split one while the paste runs and restores the original on the next run-loop turn. Settings › General › Text turns it off; Paste from the Edit menu pastes as copied. |
