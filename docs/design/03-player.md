...
| Paste, anywhere | ⌘V into any text field in the app: a pasted run that looks like title case without its spaces — `BenFolds` off a file name — gets them back, `Ben Folds` (`String.splittingTitleCaseWords`: a word starts at a capital after a lower-case letter, or at the last capital of a run of them before a lower-case letter, so `DMBLive` is `DMB Live`; `Phish`, `ACDC`, `iPhone` and anything with a space are untouched). One app-wide key monitor (`PasteTitleCaseSplitter`) sees the ⌘V first and, when the text would change, inserts the split text into the focused field itself and swallows the key — the pasteboard is never touched. The tag sheet's Paste List button asks the same rule. Settings › General › Text turns it off; Paste from the Edit menu pastes as copied. |
...
| Bindings editor, tag | Found, not scrolled to: `Find a tag to bind to <key>…` narrows the vocabulary (folded, every term, category name included); ↑ ↓ and Enter, or a click, pick — and the pick IS the bind, flagged by the Advance toggle beside the field. The field clears and the key picker steps to the next free key. `No tag matches that.` when nothing does. Each bound row carries its own `Advances` checkbox, rewritten at once. |
